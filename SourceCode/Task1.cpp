...
int main(int argc, const char** argv)
{
	...
	cout << fileName + ": I've found " << detectedFaces.size() << " faces in the image " << endl;
	cout << "Saving image with detections as Task1DetectedFaces" + fileName << endl;
	if (detectedFaces.size() > 0) {
		for (int i = 0; i < detectedFaces.size(); i++) {
			rectangle(img, detectedFaces[i].first, detectedFaces[i].second, Scalar(255, 0, 255), 3, 8, 0);
		}
	}
	imwrite("Task1DetectedFaces" + fileName, img);
